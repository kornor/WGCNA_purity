...
load(file = "Dynamic_Normal_notmerged.RData")
...
setLabels = c("Norm","Pure");
multiExpr = list(Norm = list(data = datExprNorm), Pure = list(data = datExprPure));
multiColor = list(Norm = Module1);
...
# Save the results
save(mp, file = "modulePreservation_Pure_in_Normals.RData");
load(file = "modulePreservation_Pure_in_Normals.RData")
...
write.table(pqual, "Preservation vs Quality for Pure in Normal.txt", sep = "\t")
...
mod_gene <- as.data.frame(Module1)
...
write.table(ME_pure, file = "ME_Pure_in_Normal.txt", sep = "\t")
save(PCs_pure, file = 'Pure_in_Normal_module_preservation_data.Rdata')
